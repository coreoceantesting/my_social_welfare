<x-admin.layout>
    <x-slot name="title">Pending Application List</x-slot>
    <x-slot name="heading">Pending Application List (प्रलंबित अर्जांची यादी)</x-slot>
    {{-- <x-slot name="subheading">Test</x-slot> --}}
        ...
        {{-- Take Action --}}
        <div class="row" id="takeActionContainer" style="display:none;">
            <div class="col">
                <form class="form-horizontal form-bordered" action="{{ route('changeStatus') }}" method="post" id="takeActionForm">
                    @csrf
                    <div class="card">
                        <div class="card-header">
                        ...
                        <div class="card-body py-2">
                            <input type="hidden" id="action_model_id" name="action_model_id" value="">
                            <div class="row">
                                <div class="col-md-4">
                                    <label for="remark_new" class="col-form-label">Remark (शेरा)<span class="text-danger">*</span></label>
                                    <input class="form-control" id="remark_new" name="remark_new" type="text" placeholder="Enter Remark">
                                    <span class="text-danger is-invalid remark_new_err"></span>
                                </div>
                                <div class="col-md-4">
                                    <label for="status_new" class="col-form-label">Status (स्थिती)<span class="text-danger">*</span></label>
                                    <select class="form-control" name="status_new" id="status_new">
                                        <option value="">Select Status</option>
                                        <option value="approved">Approve</option>
                                        <option value="rejected">Reject</option>
                                    </select>
                                    <span class="text-danger is-invalid status_new_err"></span>
                                </div>
                            </div>
                        </div>
...
<!-- take action -->
<script>
    $("#buttons-datatables").on("click", ".action-element", function(e) {
        e.preventDefault();
        var model_id = $(this).attr("data-id");
        var url = "{{ route('all_education_scheme.edit', ":model_id") }}";
        // console.log(url);
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                ...
                if (!data.error)
                {
                    $('#takeActionContainer').show();
                    $("#takeActionForm input[name='action_model_id']").val(data.formDetail.all_education_scheme_detail_id);
...
<script>
    // submitting form
    $('#takeActionForm').submit(function(event) {
                ...
                if (!data.error2) {
                    if (data.errors) {
                        ...
                        swal("Successful!", data.success, "success")
                            .then((action) => {
                                window.location.href = '{{ url('pending_application_list') }}';
                            });
                    }
                } else {
